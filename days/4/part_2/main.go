...
func main() {
	...
	for _, elfPair := range elfPairs {
		if elfPair[0][0] >= elfPair[1][0] && elfPair[0][0] <= elfPair[1][1] {
			lessElfs++
		} else if elfPair[0][1] >= elfPair[1][0] && elfPair[0][1] <= elfPair[1][1] {
			lessElfs++
		} else if elfPair[0][0] < elfPair[1][0] && elfPair[0][1] > elfPair[1][1] {
			lessElfs++
		}
	}
