...
func main() {
	...
	for _, command := range commands {
		...
		move := make([]string, moveNum)
		copy(move, stackCols[fromCol][0:moveNum])
		stackCols[toCol] = append(move, stackCols[toCol]...)
		stackCols[fromCol] = stackCols[fromCol][moveNum:]
		fmt.Println(stackCols)
	}
